...
## Sprint 3 — Qualidade dos dados de calibração ✅ CONCLUÍDA
...
**Tarefa 1 — Descarte de amostras de trânsito** (`calibration.ts`)
A interface `DynamicSample` teve o campo `weight` removido. Em `feedRawData`, amostras dinâmicas agora são adicionadas **somente quando `dynamicIsFixation === true`** — frames coletados durante o movimento da bolinha são descartados silenciosamente. Antes, todos os frames (trânsito + fixação) iam para o treino com pesos distintos, mas o peso era descartado em `completeDynamicCalibration`. Resultado: rótulos de trânsito (posição interpolada da bolinha ≠ posição real do olhar) eliminados completamente do conjunto de treino (G4).

**Tarefa 2 — Onset skip de 300ms + filtragem de fechamento parcial** (`calibration.ts`)
- Constante `ONSET_SKIP_MS = 300` adicionada. Na calibração estática: os primeiros 300ms após `startCollection()` são ignorados (além dos 1000ms de transição já existentes). Na calibração dinâmica: `dynamicFixationStartTime` é registrado em `pulseBall()`, e frames dentro de 300ms do início de cada fixação são descartados.
- Função `isPartialBlink(ear)`: mantém um histórico adaptativo de EAR (120 frames) e rejeita frames onde `ear < média × 0.85`. Com menos de 20 frames no histórico, não filtra (evita falsos positivos no início). Aplicada tanto em amostras estáticas quanto dinâmicas.
- `feedRawData(featuresLeft, featuresRight, ear?)` aceita o EAR como terceiro parâmetro; `main.ts` passa `ext.ear` em cada chamada.

**Tarefa 3 — λ por LOO-CV (fórmula fechada)** (`ridge.ts`)
Nova função `findBestLambda(scaledFeatures, targets)` implementada em `ridge.ts`:
- Pré-computa `ΦᵀΦ` uma vez (compartilhado entre os 8 candidatos de λ).
- Para cada λ ∈ {0.01, 0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0}:
  - Computa `A = ΦᵀΦ + λI` e `C = A⁻¹` via Gauss-Jordan (`invertMatrix`).
  - Calcula `B = C·Φᵀ` (nf × m) e a diagonal do hat matrix `H_ii = ϕ_i · B[:,i]`.
  - MSE LOO = `Σ((y_i - ŷ_i) / (1 - H_ii))² / m` — fórmula fechada, sem re-treino.
- Erro total = MSE_LOO(X) + MSE_LOO(Y); seleciona λ de mínimo. O λ escolhido é logado no console (`[IrisFlow S3] λ LOO-CV: …`).
- Em `completeDynamicCalibration`: chamada de `findBestLambda` com features LEFT escaladas; λ ótimo aplicado a ambos os modelos (LEFT e RIGHT).
- Função auxiliar `invertMatrix` (Gauss-Jordan com pivotação parcial) adicionada.

**Tarefa 4 — Clamp substituído por flag de baixa confiança** (`ridge.ts`, `calibration.ts`, `keyboard/DwellManager.ts`)
- `predictRidge` agora retorna `{ normX, normY }` (normalizados, sem clamp, sem dependência de viewport). O antigo `clmp(v) = Math.min(Math.max(v, 0), 1)` foi removido.
- Nova interface exportada `GazeResult { x, y, lowConfidence }` em `calibration.ts`.
- `mapGaze` computa `normX/Y` médio binocular, verifica se está fora do `calibHull` (bounding box dos pontos de calibração, calculado e persistido no perfil) e seta `lowConfidence = true`. O cursor ainda é clamped ao viewport físico `[0, clientWidth] × [0, clientHeight]` para não sair da tela.
- `DwellManager.update(x, y, immediateSelect, lowConfidence)`: quando `lowConfidence=true`, `requiredDwellTime = DWELL_TIME × 2` (1000ms) — o usuário olha para a borda e o teclado exige mais tempo em vez de disparar imediatamente (G7).
- `main.ts` propaga `rawGaze?.lowConfidence` para `dwellManager.update`.

**Tarefa 5 — Helper único de viewport** (`src/viewport.ts` novo, todos os arquivos)
- Novo arquivo `src/viewport.ts` exporta `getViewport(): { w, h }` usando `document.documentElement.clientWidth/Height`.
- `calibration.ts`: substituiu `window.innerWidth/Height` em `moveBallSmoothly`, `startDynamicCalibration` e `showNextPoint` (dot posicionado em px em vez de `vw/vh`).
- `ridge.ts`: removida dependência de `document.documentElement.clientWidth` (agora retorna coords normalizadas).
- `accuracy.ts`: `showValidationDot` recebe `vw, vh` do escopo externo (já via `getViewport()`); dot posicionado em px.
- Garante que calibração, predição e teste de acurácia usem exatamente a mesma área de viewport (G8).

**Tarefa 6 — Threshold de variância adaptativo** (`calibration.ts`)
- `BASE_VARIANCE_THRESHOLD = 0.0005` (mesmo valor anterior, agora explicitamente nomeado).
- Array `baselineVariances` acumula a variância máxima (left vs right) de cada ponto bem-sucedido.
- `getVarianceThreshold()`: se ≥3 pontos bem-sucedidos, usa `max(0.0005, média × 3.0)`; caso contrário, usa o base. Evita que usuários com movimento de cabeça naturalmente maior fiquem travados em loop de retry.
- `clearCalibration` e `cancelCalibration` resetam `baselineVariances` e `calibEarHistory`.

**Arquivos criados/modificados:**
- `src/viewport.ts` — novo: helper `getViewport()`
- `src/ridge.ts` — reescrito: `predictRidge` retorna `{normX,normY}`; `invertMatrix`; `findBestLambda` com LOO-CV
- `src/calibration.ts` — reescrito: todas as 6 correções integradas; exporta `GazeResult`
- `src/main.ts` — atualizado: passa `ext.ear` a `feedRawData`; propaga `lowConfidence` ao DwellManager
- `src/keyboard/DwellManager.ts` — atualizado: parâmetro `lowConfidence`; dwell 2× em zonas incertas
- `src/accuracy.ts` — atualizado: importa `getViewport`; dot em px; `showValidationDot` recebe viewport

---

---

## Sprint 4 — Head pose métrico (WebEyeTrack adaptado a cabeça estática) ✅ CONCLUÍDA
...
**Tarefa 1 — Distância via diâmetro da íris** (`extractor.ts`)
Adicionado cálculo de raio 2D da íris em coordenadas de landmarks normalizadas:
`avgIrisRadius2D = (irisRadius2DL + irisRadius2DR) / 2`. A estimativa de distância usa `distanceCm = 60.0 × 0.0074 / avgIrisRadius2D` (íris real ≈ 11.7mm; constante empírica ajustada para 60cm de referência). Campo `distanceCm: number` adicionado a `ExtractorResult` e retornado pela função.

**Tarefa 2 — `[tx, ty, tz, distanceCm]` no vetor de features** (`extractor.ts`)
Após todas as features de landmarks (x,y,z por índice + yaw/pitch/roll), os quatro valores métricos são concatenados: `featuresLeft.push(pos3D.x, pos3D.y, pos3D.z, distanceCm)` (idem para RIGHT). Quando `faceMatrix` está disponível, `pos3D = {x: faceMatrix[12], y: faceMatrix[13], z: faceMatrix[14]}` (translação métrica real da `facialTransformationMatrix`); sem matrix, usa `eyeCenter` calculado pelos landmarks. Interface `HeadPose` exportada com campos `tx, ty, tz, yaw, pitch, roll, distanceCm` para inspeção e diagnóstico.

**Tarefa 3 — Recalibração expressa inter-sessão** (`calibration.ts`)
Fluxo completo implementado em 4 etapas:
- `feedPoseFrame(pose)`: acumula 90 frames de pose pós-calibração; ao completar, chama `checkInterSessionDrift`
- `checkInterSessionDrift(avg)`: compara `Δtx`, `Δty`, `Δdist` com thresholds (0.04 em coords norm, 10cm); drift detectado → `showRecalibrationOffer()`
- `showRecalibrationOffer()`: banner não-bloqueante com botões [Recalibrar] / [Dispensar] (desaparece em 30s); elementos com `.dwell-target` para seleção por olhar
- `startExpressRecalibration()`: exibe 5 pontos (canto + centro), coleta 1s de predições brutas por ponto via `getLatestPredRaw()`, chama `fitLinear1D` (regressão 2×2 normal equations) e seta `sessionCorrection {scaleX, biasX, scaleY, biasY}`
- `getLatestPredRaw()`: lê `latestFeaturesLeft/Right` (atualizados em todo frame por `feedRawData`) e retorna a predição bruta do modelo sem a correção ativa

**Tarefa 4 — Persistência do perfil com versão** (`calibration.ts`)
- `PROFILE_VERSION = 2` introduzido; `saveProfile` persiste `version` e `refHeadPose` no JSON do `localStorage`
- `loadProfile`: se `parsed.version !== PROFILE_VERSION`, rejeita o perfil e retorna `false` (força recalibração em vez de aplicar scaler com dimensões erradas — o N de features aumentou em 4)
- `refHeadPose = averageHeadPose(calibrationPoses)` calculado ao fim de `completeDynamicCalibration` e persistido; ao carregar, `poseCheckDone = false` e `sessionCorrection = null` para iniciar nova detecção de drift
- `mapGaze`: aplica `sessionCorrection` antes do clamp de viewport se disponível: `normX = normX × scaleX + biasX`

**Integração em `main.ts`**
- `faceMatrix = new Float32Array(results.facialTransformationMatrixes?.[0]?.data)` extraído antes de `extractEyeFeatures` e passado como segundo argumento
- `calibration.feedRawData(…, ext.headPose)` — headPose acumulado durante a calibração dinâmica
- `calibration.feedPoseFrame(ext.headPose)` chamado dentro de `if (!isCalibrating && isCalibrated())` a cada frame

**Arquivos modificados:**
- `src/extractor.ts` — adicionados: interface `HeadPose`, campo `distanceCm` em `ExtractorResult`, cálculo de profundidade por íris, push de 4 valores métricos nas features, construção e retorno de `headPose`
- `src/calibration.ts` — adicionados: `PROFILE_VERSION=2`, estado Sprint 4 (`latestFeatures*`, `calibrationPoses`, `refHeadPose`, `poseBuffer`, `poseCheckDone`, `sessionCorrection`), funções `averageHeadPose`, `feedPoseFrame`, `checkInterSessionDrift`, `showRecalibrationOffer`, `startExpressRecalibration`, `fitLinear1D`, `getLatestPredRaw`; `feedRawData` aceita `headPose?`; `mapGaze` aplica correção; `saveProfile`/`loadProfile`/`clearCalibration` atualizados
- `src/main.ts` — extração de `faceMatrix`, passagem de `headPose` para `feedRawData` e `feedPoseFrame`

**Build:** ✓ 23 módulos, sem erros de tipo, 162ms

---

## Sprint 5 — Pipeline inspirado no WebEyeTrack (sem CNN)

**Status:** 🔄 A REALIZAR

**Contexto da decisão:** A implementação anterior com encoder CNN + KRR foi
revertida. O gate não passou: acurácia neural (486px) foi pior que Ridge
(407px). A precisão RMS de ambos (~334–395px) indica que o problema dominante
é de sinal, não de modelo. Nova abordagem: implementar o pipeline do
WebEyeTrack sem a camada CNN — focando em head pose métrico robusto,
features geométricas enriquecidas e adaptação por usuário.

**Objetivo:** melhorar acurácia e precisão do modo Ridge otimizado (Sprint 3)
sem depender de encoder CNN, seguindo a arquitetura de features do WebEyeTrack.

**Tarefas:** (a definir na próxima sessão de planejamento)

**Gate:** acurácia e precisão melhores que os números da Sprint 4 medidos
pelo instrumento da Sprint 1.
