...
export function isSessionLogging(): boolean { return _active; }

export function toggleSessionLog(): boolean {
  if (!_active) {
    _log = { sessionId: `irisflow_${Date.now()}`, startTime: new Date().toISOString(), frames: [] };
    _active = true;
  } else {
    _active = false;
  }
...
export function exportSessionLog(): void {
  if (!_log || _log.frames.length === 0) return;
